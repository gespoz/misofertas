...
namespace MisOfertas.Web.Controllers
{
    ...
    //        var result = await SignInManager.PasswordSignInAsync(model.User, model.Pass, model.RememberMe, shouldLockout: false);
    //        switch (result)
    //        {
    ...
    //        }
    //        var result = await UserManager.ResetPasswordAsync(user.Id, model.Code, model.Pass);
    //        if (result.Succeeded)
    //        {
    //            return RedirectToAction("ResetPasswordConfirmation", "Account");
    //        }
    ...
    //                return View("ExternalLoginConfirmation", new ExternalLoginConfirmationViewModel { User = loginInfo.DefaultUserName });
    //        }
    //    }
